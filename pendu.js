...
let shortWords = wordList.filter(item => item.length <= 4);
let mediumWords = wordList.filter(item => item.length > 4 && item.length <= 7);
let longWords = wordList.filter(item => item.length > 7); 
...
function word(level) {
    let word;
    if (level === 'easy') {
        word = getRandomWord(shortWords).toUpperCase();
        return word;
    } else if (level === 'medium') {
        word = getRandomWord(mediumWords).toUpperCase();
        return word;
    } else if (level === 'hard') {
        word = getRandomWord(longWords).toUpperCase();
        return word;
    }
}; 
...
let lifeDisplay = document.getElementById("life-number");
let test = document.getElementById("test");
let info = document.getElementById("info");
let tryDisplay = document.getElementById('try');
...
tryDisplay.style.visibility = "hidden";
lifeDisplay.style.visibility = "hidden";
...
document.getElementById('easy').onclick = () => {
    ...
    wordArray = Array.from(firstMain);
    ...
    lifeDisplay.innerHTML = life.toString();
    mainWord.style.visibility = "visible";
    info.style.visibility = 'hidden';
    tryDisplay.style.visibility = "visible";
    lifeDisplay.style.visibility = "visible";
...
function checkLetter (e) {
    for (i = 0; i < realWord.length; i++) { 
        if (e.target.innerText === realWord[i]) {
...
function endGame () {
    if (life === 0) {
        ...
        info.innerText = 'YOU LOOSE :( Choose a word length to try again!';
        tryDisplay.style.visibility = "hidden";
        lifeDisplay.style.visibility = "hidden";
        resetDisplay.style.display = "none";
}
